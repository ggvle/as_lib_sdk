package yline.application.timer;
...
public final class TimerManager
{
    ...
    public void register(String tag, float time, ITimerListener listener)
    {
        ...
        try
        {
            ...
            {
                android.util.Log.v(AppConstant.TAG_TIMER_MANAGER, "register -> tag注册");
                mTimeMap.put(tag, new TimerValueHolder().setHolder(Math.abs(time)).setListener(listener));
            }
            else
            {
                android.util.Log.v(AppConstant.TAG_TIMER_MANAGER, "register -> 该tag已经注册,更新数据");
    ...
    public void register(String tag, float time, int number, ITimerListener listener)
    {
        ...
        try
        {
            if (!mTimeMap.containsKey(tag)) // 添加tag标签
            {
                android.util.Log.v(AppConstant.TAG_TIMER_MANAGER, "register number -> tag注册");
                mTimeMap.put(tag, new TimerValueHolder().setHolder(Math.abs(time), number).setListener(listener));
            }
            else
            {
                android.util.Log.v(AppConstant.TAG_TIMER_MANAGER, "register number -> 该tag已经注册,更新数据");
    ...
    public void remove(String tag)
    {
        ...
        {
            Log.e(AppConstant.TAG_TIMER_MANAGER, "update -> 该tag还未注册,移除失败");
        }
        else
        {
            mTimeMap.remove(tag);
